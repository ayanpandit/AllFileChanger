...
import { excelConverterData } from '../data/pageContentData';
...
export default function ExcelConverter() {
  ...
  const handleConvert = async () => {
    if (!file) { setErrorMessage('Please select an Excel file'); return; }
    ...
    try {
      ...
      const response = await fetchWithWakeUp(`${API_URL}/excel-convert`, { method: 'POST', body: formData });
      ...
      a.href = url;
      a.download = `converted-${file.name.replace(/\.(xls|xlsx)$/, '')}.${format}`;
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black transition-all duration-500">
      <Helmet>
        <title>Excel Converter - Convert XLS/XLSX to CSV & More | AllFileChanger</title>
        <meta name="description" content="Convert Excel spreadsheets (XLS, XLSX) to CSV, JSON, HTML online for free. Fast, secure Excel file converter." />
      </Helmet>

      <section className="relative overflow-hidden bg-gradient-to-br from-green-600 via-emerald-600 to-teal-600 dark:from-black dark:via-black dark:to-black">
        <div className="absolute inset-0 bg-black/20 dark:bg-black/80 transition-all duration-500"></div>
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-24">
          <div className="text-center">
            <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-green-500 to-emerald-600 rounded-3xl mb-6 shadow-2xl hover:scale-110 transition-all duration-300">
              <span className="text-3xl text-white">📊</span>
            </div>
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4 sm:mb-6">
              <span className="text-yellow-300 dark:text-yellow-400">Excel Converter</span>
            </h1>
            <p className="text-lg sm:text-xl text-green-100 dark:text-gray-300 mb-6 max-w-3xl mx-auto">Convert Excel spreadsheets to CSV, JSON, HTML. Fast, secure, and free.</p>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white dark:bg-black rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden transition-all duration-500">
            <div className="p-4 sm:p-6 lg:p-8">
              <div className={`relative border-2 border-dashed rounded-xl p-6 sm:p-8 lg:p-12 text-center transition-all duration-300 ${dragActive ? 'border-green-500 bg-green-50 dark:bg-green-950/50 scale-105' : 'border-gray-300 dark:border-gray-800 hover:border-green-400 dark:hover:border-green-600'}`} onDrop={handleDrop} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver}>
                <input type="file" onChange={handleFileInput} accept=".xls,.xlsx" className="hidden" id="file-upload" />
                <div className="space-y-4 sm:space-y-6">
                  <div className="mx-auto w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-300">
                    <span className="text-3xl sm:text-4xl">📈</span>
                  </div>
                  <div>
                    <label htmlFor="file-upload" className="cursor-pointer text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white hover:text-green-600 dark:hover:text-green-400 transition-colors duration-200">
                      {file ? file.name : 'Choose an Excel file or drag it here'}
                    </label>
                    <p className="mt-2 text-sm sm:text-base text-gray-500 dark:text-gray-400">Supports XLS and XLSX formats (max 50MB)</p>
                  </div>
                  <div className="flex flex-wrap justify-center gap-2">
                    <span className="px-3 py-1 bg-gray-100 dark:bg-black rounded-full transition-colors duration-300">XLS</span>
              ...
              {file && (
                <div className="mt-6 space-y-4">
                  <div>
                    ...
                    <select value={format} onChange={(e) => setFormat(e.target.value)} className="w-full p-3 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-black text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200">
          ...
          <section className="mt-12 sm:mt-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { icon: '⚡', title: 'Lightning Fast', desc: 'Convert spreadsheets instantly' },
                { icon: '🔒', title: '100% Secure', desc: 'Files deleted after conversion' },
                { icon: '🆓', title: 'Completely Free', desc: 'No limits, no registration' },
              ].map((feature, i) => (
                <div key={i} className="group bg-white dark:bg-black rounded-xl p-6 shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-200 dark:border-gray-800 hover:border-green-200 dark:hover:border-green-600 hover:-translate-y-1">
          ...
          <section className="mt-12 sm:mt-16">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                <Link key={i} to={tool.path} className="group bg-white dark:bg-black rounded-xl p-4 shadow-md hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-800 hover:border-green-200 dark:hover:border-green-600 hover:-translate-y-1">
      ...
      <PageContent {...excelConverterData} />
    </div>
  );
}
